...
export const HOW_TO_SECTIONS = [
  {
    ...
    title: 'Quick start',
    body: [
      'Pick a **reef** in the header — try **Shallow reef** first.',
      'Choose a **dock** — first visit opens **Chart status**; the dock rail shows **Chart status · Map · Actions**.',
  ...
  },
  {
    ...
    terms: [
      ...
      { name: 'Dock', desc: 'A channel with its own folder under `raw/` (e.g. ⚓ My Portfolio → `raw/papers/`).' },
      ...
      { name: 'Map', desc: 'Charted items for the active dock — themes, concepts, syntheses; PDF linked in the paper row.' },
      { name: 'Dock workspace', desc: 'Sticky rail under docks: switch Chart status / Map / Actions for the selected dock. Guide resets per reef.' },
