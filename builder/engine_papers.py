...
def build(root, deepdives_dir, data, today, rebuild_papers=None, rebuild_themes=None):
    ...
    for p in P:
        ...
        if vf and vf.get("llm_enriched") and not vf.get("human_verified"):
            src = vf.get("enrichment_source") or "local-32b"
            model = vf.get("llm_model") or "LLM"
            src_label = {
                "local-32b": "local 32B model",
                "local-custom": f"local model ({model})",
                "frontier": f"frontier model ({model})",
            }.get(src, model)
            lines += [
                f"> [!warning] Uncharted — needs human review\n"
                f"> Deep Dive was generated by **{src_label}**. "
                "Territory is **uncharted** until you read and mark **verified** in Portolan.",
                "",
            ]
        elif vf and vf.get("human_verified") and vf.get("llm_enriched"):
            by = vf.get("verified_by") or "reviewer"
            when = f" on {vf['verified_at']}" if vf.get("verified_at") else ""
            lines += [f"> [!tip] Human verified\n> Reviewed by {by}{when}.", ""]
