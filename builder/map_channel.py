...
updates the auto registry (auto_papers.json or auto_sources.json).
...
def _load_auto(builder_dir, auto_file, auto_key=None):
    return registry.load(builder_dir, auto_file)
...
def _write_auto(builder_dir, entries, auto_file, auto_key=None, channel_id=None):
    # JSON, not generated Python: never executed on load, round-trips strings losslessly.
    registry.save(builder_dir, auto_file, entries)


def _merge_auto_sources(builder_dir, channel_id, new_entries, auto_file, auto_key):
    """Keep entries from other channels in shared auto_sources.json."""
...
def map_channel(vault, channel_id, dry_run=False):
    ...
    if profile == "portfolio":
        if registry.json_name(auto_file) == "auto_lab_papers.json":
